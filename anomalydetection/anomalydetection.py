...
def timeseries_dataset(data, time_steps, seq_stride,
    ...
    indices = tf.data.Dataset.zip((tf.data.Dataset.range(len(start_positions)), positions_ds)).map(
        tf.autograph.experimental.do_not_convert(lambda i, positions: tf.range(positions[i], positions[i] + (time_steps + warm_up))),
        num_parallel_calls=tf.data.AUTOTUNE)
    # create dataset
    ...
    if targets is not None:
        ...
        if data_is_target:
            if warm_up is None:
                ...
                indices = tf.data.Dataset.zip((tf.data.Dataset.range(len(start_positions)), positions_ds)).map(
                    tf.autograph.experimental.do_not_convert(lambda i, positions: tf.range(positions[i], positions[i] + time_steps)),
                    num_parallel_calls=tf.data.AUTOTUNE)
